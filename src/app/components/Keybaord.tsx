...
const fingerColors: { [key: string]: string } = {
  "left-pinky": "bg-red-600",
  "left-ring": "bg-orange-600",
  "left-middle": "bg-yellow-600",
  "left-index": "bg-green-600",
  "right-index": "bg-blue-600",
  "right-middle": "bg-indigo-600",
  "right-ring": "bg-purple-600",
  "right-pinky": "bg-pink-600",
  none: "bg-cyan-950",
};

// add slight padding to each row to resemble traditional keybaord
const rowMarign: { [key: number]: string } = {
    0: "",
    1: "ms-4",
    2: "ms-8",
    3: "",
  ...
  return (
    <div className="flex flex-col items-center p-4">
      {rows.map((row, rowIndex) => (
        <div key={rowIndex} className={classNames("flex",rowMarign[rowIndex])}>
          {row.map(renderKey)}
        </div>
      ))}
    </div>
